...
def _conv_block(inputs,filers,alpha,kernel=(3,3),strides=(1,1)):
...
def _depthwise_conv_block(inputs,pointwise_conv_filters,alpha,
                          deph_multiplier=1,strides=(1,1),block_id=1):

   channel_axis=1 if IMAGE_ORDERING=='channels_first' else -1
   pointwise_conv_filters=int(pointwise_conv_filters*alpha)

   x=ZeroPadding2D((1,1),data_format=IMAGE_ORDERING,name='conv_pad_%d' % block_id)(inputs)
   x=Deconvolution2D((3,3),data_format=IMAGE_ORDERING,
                     padding='valid',
                     deph_multiplier=deph_multiplier,
                     strides=strides,
                     use_bias=False,
...
def get_mobilenet_encoder(input_height=224,input_width=224,pertrained='imagenet'):
